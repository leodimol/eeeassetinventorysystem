...
const AnalyticsDashboard = ({ equipment, filters, compact }) => {
  ...
  const comprehensiveMetrics = useMemo(() => {
    ...
    const available = filteredEquipment.filter(
      (item) => item.status?.toLowerCase() === 'available'
    ).length;
    const inUse = filteredEquipment.filter((item) =>
      ['in_use', 'loaned', 'active'].includes(item.status?.toLowerCase())
    ).length;
    const maintenance = filteredEquipment.filter((item) =>
      ['maintenance', 'damaged'].includes(item.status?.toLowerCase())
    ).length;
    const retired = filteredEquipment.filter(
      (item) => item.status?.toLowerCase() === 'retired'
    ).length;
    ...
    // Condition metrics
    const excellent = filteredEquipment.filter(
      (item) => item.condition?.toLowerCase() === 'excellent'
    ).length;
    const good = filteredEquipment.filter(
      (item) => item.condition?.toLowerCase() === 'good'
    ).length;
    const fair = filteredEquipment.filter(
      (item) => item.condition?.toLowerCase() === 'fair'
    ).length;
    const poor = filteredEquipment.filter(
      (item) => item.condition?.toLowerCase() === 'poor'
    ).length;

    // Assignment metrics
    const assigned = filteredEquipment.filter(
  ...
  const hubData = useMemo(() => {
    ...
    let result = Object.entries(hubCounts)
      .map(([name, value]) => ({
        name: name,
        value,
  ...
  const conditionData = useMemo(() => {
    ...
    let result = Object.entries(conditionCounts)
      .map(([name, value]) => ({
  ...
  const assetTypeData = useMemo(() => {
    ...
    let result = Object.entries(typeCounts)
      .map(([name, value]) => ({
        name: name.charAt(0).toUpperCase() + name.slice(1),
        value,
  ...
  const renderComprehensiveDashboard = () => (
    <div className="space-y-4">
                  ...
                  <div key={index} className="space-y-1">
                    ...
                    <div className="h-1 bg-[var(--bg-tertiary)] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${comprehensiveMetrics.total > 0 ? (item.value / comprehensiveMetrics.total) * 100 : 0}%`,
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                ...
                <span className="text-sm font-bold text-emerald-400">
                  {comprehensiveMetrics.underWarranty}
                </span>
              </div>
              <div className="flex items-center justify-between p-2 rounded-lg bg-orange-500/10 border border-orange-500/20">
                ...
                <span className="text-sm font-bold text-orange-400">
                  {comprehensiveMetrics.expiredWarranty}
                </span>
              </div>
              <div className="flex items-center justify-between p-2 rounded-lg bg-gray-500/10 border border-gray-500/20">
                ...
                <span className="text-sm font-bold text-gray-400">
                  {comprehensiveMetrics.noWarranty}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[var(--bg-glass-light)] rounded-lg p-4 border border-[var(--border-glass)]">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between p-2 rounded-lg bg-gray-500/10 border border-gray-500/20">
                ...
                <span className="text-sm font-bold text-gray-400">
                  {comprehensiveMetrics.unassigned}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {recentActivity.length === 0 ? (
                ...
                recentActivity.map((item, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 p-2 rounded-lg bg-[var(--bg-glass)] hover:bg-[var(--bg-tertiary)] transition-all"
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-[var(--bg-glass-light)] rounded-lg p-4 border border-[var(--border-glass)]">
          <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-3">
            Equipment Types ({assetTypeData.length})
          </h3>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="bg-[var(--bg-glass-light)] rounded-lg p-4 border border-[var(--border-glass)]">
        <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-3">
          All Equipment Types Breakdown
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
          {assetTypeData.map((type, index) => (
            <div
              key={index}
              className="p-3 rounded-lg bg-[var(--bg-glass)] border border-[var(--border-glass)] hover:border-[var(--border-color)] transition-all"
            >
              ...
              <p className="text-[9px] text-[var(--text-tertiary)] mt-1">
                {comprehensiveMetrics.total > 0
                  ? Math.round((type.value / comprehensiveMetrics.total) * 100)
                  : 0}
                % of total
              </p>
            </div>
          ...
          <div className="flex flex-wrap justify-center gap-2 text-sm">
            {utilizationData.map((entry, index) => (
              <div
                ...
                className="flex items-center gap-2 px-2 py-1 rounded-md border border-[var(--border-glass)] bg-[var(--bg-glass-light)]"
              >
                <span
